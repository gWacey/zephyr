...
#define TAC5112_REG_DAC_CH1A_CFG0     TAC5112_REG(0, 0x67) /* DVOL, ch1 (left) */
#define TAC5112_REG_DAC_CH1B_CFG0     TAC5112_REG(0, 0x69) /* DVOL, ch1 (left) */
#define TAC5112_REG_DAC_CH2A_CFG0     TAC5112_REG(0, 0x6E) /* DVOL, ch2 (right) */
#define TAC5112_REG_DAC_CH2B_CFG0     TAC5112_REG(0, 0x70) /* DVOL, ch2 (right) */
 ...
 */
#define TAC5112_DAC_VOL_MIN_HALF_DB \
	((int32_t)TAC5112_DAC_DVOL_MIN - (int32_t)TAC5112_DAC_DVOL_0DB)
#define TAC5112_DAC_VOL_MAX_HALF_DB \
	((int32_t)TAC5112_DAC_DVOL_MAX - (int32_t)TAC5112_DAC_DVOL_0DB)
#define TAC5112_ADC_VOL_MIN_HALF_DB \
	((int32_t)TAC5112_ADC_DVOL_MIN - (int32_t)TAC5112_ADC_DVOL_0DB)
#define TAC5112_ADC_VOL_MAX_HALF_DB \
	((int32_t)TAC5112_ADC_DVOL_MAX - (int32_t)TAC5112_ADC_DVOL_0DB)
 ...
 * registers (Section 8.1.3.24-8.1.3.34) for applications that must use a
 * clock ratio outside of the auto-detected table (Table 7-7 / 7-8) and
 * have derived P/J/D/N/M values from TI's "Clocking Configuration of
 * Device and Flexible Clocking For TAx5x1x Family" application note.
 * This driver does not compute those values itself: it only validates
 * that each field fits within its documented register range and writes
 * them verbatim, since the closed-form PLL frequency equation is not
 * reproduced in the register-level section of the datasheet consulted
 * while writing this driver.
 */
 ...
 * take dev->data's lock itself; it is not exposed outside this driver
 * precisely to keep that an explicit, deliberate choice rather than an
 * implicit dependency.
 */
...
int tac5112_reg_update(const struct device *dev, struct tac5112_reg reg, uint8_t mask,
			uint8_t val);
