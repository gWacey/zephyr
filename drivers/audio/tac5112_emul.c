 ...
 * real driver code without hardware.
 *
...
#define TAC5112_EMUL_NUM_PAGES 4U

struct tac5112_emul_data {
	struct i2c_emul emul;
	const struct device *i2c;
	uint8_t regs[TAC5112_EMUL_NUM_PAGES][256];
	uint8_t cur_page;
	uint8_t cur_addr;
};
...
static void tac5112_emul_reset_defaults(struct tac5112_emul_data *data)
{
	...
	/* Page 3 reset defaults (Section 8.1.3.27-8.1.3.32). */
...
{
	uint8_t val = data->regs[page][addr];

	if (page == 1U) {
		/* Latched fault/interrupt registers self-clear on read
		 * (datasheet Sections 8.1.2.24-8.1.2.26).
		 */
