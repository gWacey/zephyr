 ...
 *
 * Conformance notes:
 *  - Implements the mandatory members of `struct audio_codec_api`
...
static int tac5112_configure_dai(const struct device *dev, const struct audio_codec_cfg *cfg)
{
	...
	if ((i2s->frame_clk_freq < 4000U) || (i2s->frame_clk_freq > 768000U)) {
...
static int codec_clear_errors(const struct device *dev)
{
	 ...
	 * (datasheet Sections 8.1.2.24, 8.1.2.25, 8.1.2.26) and de-assert the
	 * codec's fault/interrupt output when all latched conditions have
	 * cleared.
	 */
